...
const { CLI, log } = require('roar');
const debug = require('debug')('hcp:template');
...
export class Template {
  ...
  constructor (options = {}) {
    ...
    this.argv = this.options.argv = this.options.argv || {};
  ...
  init (files = []) {
    ...
    return Promise.all(this.files.map(file => this.write(file, this.dest)))
  ...
  copy (file) {
    ...
    debug('copy file %s to %s', file, dest);
  ...
  placeholders (content = '') {
    ...
    placeholders = placeholders.filter((name) => {
      return !(name in this.options.argv || name in this.env);
    });
