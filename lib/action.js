...
action.renderDir = renderDir;

function action(file, opts, done) {
  start = Date.now();
  var stat = fs.statSync(file);
  var content = stat.isFile() && fs.readFileSync(file, 'utf8');

  if (!content && stat.isDirectory()) {
    return renderDir.call(this, file, opts, end(done));
  }

  return render.call(this, file, opts, end(done));
}
...
function render(content, opts, done) {
  ...
  var questions = placeholders.map(function(p) {
    ...
    return {
      ...
      message: message,
      name: p,
      default: opts.options[p] || path.basename(name)
    }
  });
  ...
  this.prompt(questions, function(answers) {
    ...
    placeholders.forEach(function(p) {
      var answer = answers[p] === 'true' ? true :
        answers[p] === 'false' ? false :
        answers[p];

      data[p] = typeof answer !== 'undefined' ? answer : (opts.options[p] || p);
    }, this);
...
function renderFiles(files, opts, done) {
  ...
  var promises = files.map(function(file) {
    ...
    return fs.readFile(srcfile, 'utf8').then(function(content) {
      ...
      if (placeholders) placeholders = placeholders.map(function(p) {
        return p.replace(/{{\s*/, '').replace(/\s*}}/, '');
      });
      ...
      if (placeholders) placeholders.forEach(function(p) {
...
function parseOutput(content, data) {
  ...
  var matches = body.match(/^(\/\/|<!--)\s+output:\s*([^\s]+)/);
